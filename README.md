...
Pagely 플랫폼의 결제 마이크로서비스입니다. 중고 도서 거래의 결제 생성·승인·정산 보류금 관리를 담당하며,
...
- 결제 상태(`payment_status`), 보류금 상태(`payment_hold_status`), 정산 상태(`settlement_status`)는
  PostgreSQL 네이티브 ENUM으로 정의되어 있고, 엔티티에서 `@JdbcTypeCode(SqlTypes.NAMED_ENUM)`로 매핑합니다.
...
- **Lombok** — 보일러플레이트 축소.
- **Bean Validation** (`spring-boot-starter-validation`) — 요청 DTO 검증.
...
|---|---|---|---|
| `POST` | `/api/v1/payments/confirm` | Toss 결제 승인 요청 | `@AuthRequired(role = Role.USER)` |
...
- `paymentKey`(필수), `orderId`(필수, UUID), `amount`(필수, 0 이상) — Bean Validation으로 검증
- 구매자 ID는 요청 본문이 아니라 `@CurrentUserId`로 인증 컨텍스트에서 주입받습니다.

**Response** — `ApiResponse`로 감싼 `ConfirmPaymentResponse`
...
```json
{
  ...
  "pgProvider": null,
  "paymentKey": "...",
  "pgApprovedAt": "2026-09-03T14:20:11"
}
```

<!-- TODO: 결제 취소/환불 API(PaymentProvider.cancel)는 인터페이스만 정의되어 있고 미구현 상태입니다. -->
...
- `p_payment.order_id`에 UNIQUE 제약(`V2__add_unique_order_id.sql`)이 있어
...
| 버전 | 내용 |
|---|---|
| `V1__init.sql` | ENUM 타입 3종 + 테이블 4종(`p_payment`, `p_payment_history`, `p_payment_hold`, `p_settlement`) 생성 |
| `V2__add_unique_order_id.sql` | `p_payment.order_id` UNIQUE 제약 추가 |
| `V3__add_confirm_requested_payment_status.sql` | `payment_status` ENUM에 `CONFIRM_REQUESTED` 추가 |

운영 환경은 `ddl-auto: validate`이므로 스키마 변경은 반드시 Flyway 마이그레이션으로 추가해야 합니다.

## 트러블슈팅

### 주문 및 결제 처리 흐름
- Payment Service가 Market Service와 Kafka 메시지로 통신하는 이벤트 기반 구조로
  결제를 처리.
- 흐름: ① 사용자가 구매 버튼 클릭 → ② Toss 결제 위젯을 통해 결제 요청 진행 →
  ③ 결제 승인 완료 처리. Payment Service는 Market Service의 주문 생성 이벤트를
  수신해 결제 정보를 생성하고, 결제 결과를 이벤트로 발행함.

### PG 결제 승인 실패 처리 흐름 — Feign ErrorDecoder
...
<!-- TODO: 결제 위젯 및 승인 완료 화면 스크린샷 경로를 채워주세요 -->
| 결제 요청 | 결제 승인 완료 |
|---|---|
| ![결제 요청 화면](docs/images/payment-request.png) | ![결제 승인 완료 화면](docs/images/payment-complete.png) |

### 결제 승인 플로우 데모

<!-- TODO: 결제 승인 전체 플로우 GIF 또는 시연 영상 링크를 채워주세요 -->
![결제 승인 플로우](docs/images/payment-confirm-flow.gif)

### 아키텍처 다이어그램

<!-- TODO: 서비스 아키텍처 / 결제 시퀀스 다이어그램 이미지를 채워주세요 -->
![결제 서비스 아키텍처](docs/images/payment-architecture.png)
...
| 배포 서비스 | <!-- TODO: 배포 URL --> |
| 시연 영상 | <!-- TODO: 시연 영상 링크 --> |
| API 문서 | <!-- TODO: Swagger / Postman 문서 링크 --> |
| 프로젝트 노션 / 회고 | <!-- TODO: 링크 --> |
